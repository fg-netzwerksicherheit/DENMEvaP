...
package jEsperToolkit.test.fixed_rate.flat.maps;
 ...
 * 
 */
public class UdpFilterFixedRateTests extends TestWithFixedEventSendRate {
	...
	public void setup() {
		...
		currentIndex = 0;
		List<Map<?, ?>> data = CljNetPcapJavaAdapter.extractMapsFromPcapFile("test/jEsperToolkit/test/benchmark/test_data/combined_office_22-04-2013.pcap");
        for (int i = 0; i < 15; i++) {
            events.addAll(data);
        }
        
        erf.addEventType("sniffer.header.parsed.flat", (Map<String, Object>) events.get(490));
        
        erf.createStatement("INSERT INTO cep.udp SELECT * FROM sniffer.header.parsed.flat WHERE udpSrc is not null");
        
        setCountedEventTypes("sniffer.header.parsed.flat", "cep.udp");

		setEventSender(new TestEventSender() {
			@Override
			public void sendEvents(EsperRuntimeFacade erf) {
				synchronized (this) {
					erf.send("sniffer.header.parsed.flat",
	...
	public static void main(String args[]) throws UnsupportedEncodingException,
		...
		UdpFilterFixedRateTests test = new UdpFilterFixedRateTests();
	...
	private static void printEventStats(UdpFilterFixedRateTests test) throws FileNotFoundException {
