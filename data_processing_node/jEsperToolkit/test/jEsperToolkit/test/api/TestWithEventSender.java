 ...
 *
 */
public class TestWithEventSender extends TestWithEsperRuntimeFacade {

    protected static List<Object> events = new ArrayList<Object>();
	
	private TestEventSender eventSender;

	/**
	 * Set the {@link TestEventSender} to be used. The event sender must be
	 * set prior to executing the actual benchmark via
	 * {@link TimedBenchmark#runSimpleBenchmark()} and prior to preparing the
	 * benchmark via {@link TimedBenchmark#prepareBenchmark(String...)}.
	 * 
	 * @param eventSender
	 */
	public void setEventSender(TestEventSender eventSender) {
		this.eventSender = eventSender;
	}
	
	protected void sendEvents() {
		assertNotNull("Event sender must be set! "
				+ "Please set it via setEventSender(eventSender).", eventSender);
		eventSender.sendEvents(erf);
	}
