...
package jEsperToolkit.test.benchmark.insert_into;
...
public class SimpleInsertIntoWithComparisonBenchmark2 extends TimedBenchmark {
    ...
    public void setup() {
        super.setup();
        erf.addEventType("A", (Map<String, Object>) events.get(0));
        
		erf.createStatement("INSERT INTO B SELECT * FROM A WHERE u = 0");
		erf.createStatement("INSERT INTO C SELECT * FROM B WHERE v = 0");
		erf.createStatement("INSERT INTO D SELECT * FROM C WHERE w = 0");
		erf.createStatement("INSERT INTO E SELECT * FROM D WHERE x = 0");
		erf.createStatement("INSERT INTO F SELECT * FROM E WHERE y = 0");
		erf.createStatement("INSERT INTO G SELECT * FROM F WHERE z = 0");
        
        setEventSender(new TestEventSender() {
            @Override
            public void sendEvents(EsperRuntimeFacade erf) {
                erf.sendAll("A", events);
            }
        });
        
        prepareBenchmark("A", "B", "C", "D", "E", "F", "G");
    }

    @Test
    public void simpleMapEventBenchmark() throws InterruptedException {
        System.out.println("SimpleInsertIntoBenchmark Begin");
        for (int i = 0; i < 30; i++) {
            runSimpleBenchmark();
        }
        System.out.println("SimpleInsertIntoBenchmark End");
    }
