...
public class SimpleEventMapBenchmarks extends TimedBenchmark {
    ...
    public static void setupClass() {
        for (int i = 1; i <= 100000; i++) {
            ...
            event.put("number", i);
            event.put("string", String.valueOf(i));
            events.add(event);
        }
    }
    ...
    public void setup() {
        super.setup();
        erf.addEventType("SimpleMapEvent", (Map<String, Object>) events.get(0));
        
        setEventSender(new TestEventSender() {
            @Override
            public void sendEvents(EsperRuntimeFacade erf) {
                erf.sendAll("SimpleMapEvent", events);
            }
        });
        
        prepareBenchmark("SimpleMapEvent");
    }

    @Test
    public void simpleMapEventBenchmark() throws InterruptedException {
        System.out.println("simpleMapEventBenchmark Begin");
        for (int i = 0; i < 30; i++) {
            runSimpleBenchmark();
        }
        System.out.println("simpleMapEventBenchmark End");
    }
