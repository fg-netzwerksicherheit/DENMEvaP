 ...
 *
 */
public class SimpleSnifferDataBenchmark extends TimedBenchmark {

    @BeforeClass
    public static void setupClass() {
    	@SuppressWarnings("unchecked")
		List<Map<?, ?>> data = CljNetPcapJavaAdapter.extractNestedMapsFromPcapFile("test/jEsperToolkit/test/clj_net_pcap/icmp-echo-request.pcap");
        for (int i = 1; i <= 100000; i++) {
            events.add(data.get(0));
        }
    }
    ...
    public void setup() {
        ...
        prepareBenchmark("sniffer.header.parsed");
    }

    @Test
    public void simpleSnifferEventBenchmark() throws InterruptedException {
        System.out.println("simpleSnifferEventBenchmark Begin");
        for (int i = 0; i < 30; i++) {
            runSimpleBenchmark();
        }
        System.out.println("simpleSnifferEventBenchmark End");
    }
