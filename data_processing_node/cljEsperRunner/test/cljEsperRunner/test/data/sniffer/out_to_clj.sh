...
echo -ne "Removing raw packet data.\t\t"
colorize_output sed -i '/^Packet*/d' "$TEMP"
echo ""

echo -ne "Removing status output.\t\t"
colorize_output_short sed -i '/^Starting*/d' "$TEMP"
colorize_output_short sed -i '/^{:interface*/d' "$TEMP"
colorize_output_short sed -i '/^{:help*/d' "$TEMP"
colorize_output_short sed -i '/^Type*/d' "$TEMP"
colorize_output_short sed -i '/^cljNetPcap*/d' "$TEMP"
colorize_output_short sed -i '/^Removing*/d' "$TEMP"
colorize_output sed -i '/^Leaving*/d' "$TEMP"
echo ""

echo -ne "Adding event headers and footers.\t\t"
colorize_output_short sed -i 's/"PcapHeader"/"sniffer.header.parsed" {"PcapHeader"/' "$TEMP"
colorize_output sed -i 's/}}/}}}/' "$TEMP"
echo ""
...
(ns 
  ^{:author "Ruediger Gad",
    ...
    :raw-file "%RAW"
    :date "%DATE"}
   cljEsperRunner.test.data.sniffer.%NAME)
